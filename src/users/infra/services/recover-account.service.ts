...
export class RecoverAccountService {
	...
	async run(email: string): Promise<OTP> {
		...
		const sender = (await this.emailProvider.init())
			.setFrom({
				name: 'Fluffy Umbrela Finances',
				email: 'suport@fluffy.com'
			})
			.setTo(email)
	...
	private getLink(otp: OTP): string {
		...
		const url_email = encodeURIComponent(otp.email);
		return `${host}/recuperar-senha?otp=${otp.otp}&amp;timestamp=${
			otp.timestamp
		}&email=${url_email}&hash=${otp.generate()}`;
	}
	...
	private recoverHtml(otp: OTP): string {
		...
		return (
			`Seu link de recuperação de conta está aqui:<br /><br />` +
			`<a href="${link}">${link}</a>`
		);
	}
}
